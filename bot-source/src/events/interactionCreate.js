        ...
        if (interaction.isModalSubmit()) {
            try {
                ...
                const container = new ContainerBuilder()
                    ...
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent('### <:custom_cross:1524603300752785550> 處理失敗'))
...
async function _handleValUrlLogin(interaction, bot) {
    ...
    if (result.error) {
        const container = new ContainerBuilder()
            .setAccentColor(Colors.Error)
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### <:custom_cross:1524603300752785550> 登入失敗`))
            .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### <:custom_check:1524601509772529665> 登入成功！`))
        .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
