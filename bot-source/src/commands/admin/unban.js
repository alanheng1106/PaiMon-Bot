...
module.exports = {
    ...
    category: 'admin',
    helpText: '🔹 `/unban [使用者ID] [原因]` - 解除封鎖已被 ban 的使用者 (需輸入 ID)',
    async execute(interaction, bot) {
        ...
        if (!/^\d{17,20}$/.test(userId)) {
            return bot.sendError(
                interaction,
                '格式錯誤',
                '請輸入有效的使用者 ID (17-20 位數字).\n> 在 Discord 開啟開發者模式, 對使用者按右鍵就能複製 ID.'
            );
        }
        ...
        if (!banEntry) {
            return bot.sendError(interaction, '未在封鎖名單', `使用者 ID \`${userId}\` 不在這個伺服器的封鎖名單中.`);
        }
        ...
        try {
            ...
            bot.sendError(interaction, '執行失敗', '解封時出了點問題, 請稍後再試.');
        }
    }
};
