...
module.exports = {
    ...
    async execute(interaction, bot) {
        ...
        if (user.id === interaction.guild.ownerId) return bot.sendError(interaction, '權限受限', '你不能禁言伺服器擁有者！');
        ...
        await user.send(`⏳ 你已在 **${interaction.guild.name}** 被禁言 **${durationLabel}**。\n📋 原因：\`${reason}\``).catch(() => {});

        try {
            await member.timeout(durationSeconds * 1000, `By ${interaction.user.tag}: ${reason}`);
            await bot.sendSuccess(interaction, '⏳ 禁言成功',
                `✅ 已成功禁言 **${user.tag}**\n⏱️ 時長：**${durationLabel}**\n📋 原因：\`${reason}\``);
        } catch (err) {
