    ...
    async execute(interaction, bot) {
        ...
        if (user.id === bot.user.id) {
            return bot.sendError(
                interaction,
                '操作無效',
                '我不太建議改我自己的暱稱! 如果需要, 請到伺服器設定手動修改'
            );
        }
        if (user.id === interaction.guild.ownerId) {
            return bot.sendError(interaction, '權限受限', '你不能改伺服器擁有者的暱稱!');
        }
        ...
        try {
            ...
            await bot.sendSuccess(
                interaction,
                '📝 暱稱變更成功',
                `已將 **${user.tag}** 的暱稱從 \`${oldNickname}\` 改為 \`${nickname}\``
            );
        } catch (err) {
            console.error('[Rename CMD]', err);
            bot.sendError(interaction, '執行失敗', '改暱稱時出了點問題, 請稍後再試.');
        }
    }
};
