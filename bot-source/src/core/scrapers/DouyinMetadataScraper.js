...
class DouyinMetadataScraper extends BaseMetadataScraper {
    ...
    async scrape(url, signal) {
        try {
            ...
            const item = pageData?.videoInfoRes?.item_list?.[0] || pageData?.videoInfoRes?.itemStruct || pageData?.videoData;
            ...
            let video = item.video?.play_addr?.url_list?.[0] || item.video?.playAddr?.[0]?.src;
